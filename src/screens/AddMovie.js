...
const AddMovie = ({ route }) => {
  const editMovie = route.params;
  const navigation = useNavigation();
  const [step, setStep] = useState(1);
  const [movie, setMovie] = useState(editMovie?.movie || '');
  const [image, setImage] = useState(editMovie?.image || '');
  ...
  const { saveMovie, fetchMovies } = useStore();
  ...
  const handleNextStep = () => {
    if (step === 2) {
      const newMovie = { id: Date.now(), movie, image, rating, description };
      saveMovie(newMovie, editMovie);
                          ...
                          >
                            ...
                            >
                              {star <= rating ? (
                                ...
                                <Image
                                  source={require('../assets/icons/ratingBlured.png')}
                                />
                              )}
                            </View>
                          </TouchableOpacity>
                  ...
                  <View style={styles.resContainer}>
                    ...
                    <View style={styles.buttonsWrap}>
                      <Text style={styles.notificationText}>
                        {step === 1 &&
                          `In the next step, add a photo and the name of the movie you watched.`}
                        {step === 2 &&
                          `At this stage, you need to give a rating and add a short description.`}
                      </Text>
                      <SmallButton
                        title={'Ok'}
                        height={{ height: 20 }}
